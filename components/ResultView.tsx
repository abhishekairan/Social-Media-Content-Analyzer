...
export const ResultView: React.FC<ResultViewProps> = ({ result, onBack }) => {
	...
	return (
		<div className="w-full h-full">
			{/* Header */}
			<div className="sticky top-0 bg-white border-b border-gray-200 p-4 flex flex-col md:flex-row items-center justify-between gap-5">
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6 h-[calc(100%-80px)] overflow-hidden">
				...
				<div className="flex flex-col bg-gray-50 rounded-lg border border-gray-200 overflow-hidden">
					<div className="p-4 border-b border-gray-200 bg-white">
						<p className="text-sm font-semibold text-gray-700">{result.fileName}</p>
					</div>
					<div className="flex-1 overflow-auto p-4 flex items-center justify-center">
						{result.fileType === 'pdf' ? (
							<embed
								type="application/pdf"
								src={result.previewUrl + '#toolbar=0&navpanes=0&scrollbar=0'}
								className="w-full h-full min-h-[600px] rounded-lg border-none"
								title="PDF Preview"
							/>
						) : (
							<Image
